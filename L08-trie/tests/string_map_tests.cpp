...
TEST(string_map_test, copiarDelMarian) {
    string_map<int> aCopiar;

    aCopiar.insert(make_pair("\n\v\f", 999));


    string_map<int> copia(aCopiar);

    EXPECT_TRUE(copia.count("\n\v\f") == 1);
    EXPECT_TRUE(aCopiar.count("\n\v\f") == 1);
    EXPECT_EQ(aCopiar.at("\n\v\f"), copia.at("\n\v\f"));
}
