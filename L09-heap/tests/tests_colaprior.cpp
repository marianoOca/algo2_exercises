...
TEST(ColaPrior, testDecente) {
    for (int t = 0 ; t <= tope ; t++) {
        ColaPrior<int> p, q;
        vector<int> v;
        // armamos un vector ordenado de menor a mayor
        for (int j = 0; j <= t; j++) v.push_back(j);

        // le insertamos los mismos elementos en orden a la cola
        for (int j = 0; j <= t; j++) q.encolar(v[j]);

        // le insertamos los mismos elementos de mayor a menor a otra cola
        for (int j = t; j >= 0; j--) p.encolar(v[j]);

        // chequeamos que estén bien los tamaños
        ASSERT_EQ(q.tam(), t+1);
        ASSERT_EQ(p.tam(), t+1);

        // limpiamos las pilas chequeando que estén bien ordenadas las colas
        for (int j = t; j >= 0; j--) {
            ASSERT_EQ(q.proximo(), j);
            ASSERT_EQ(p.proximo(), j);
            q.desencolar();
            p.desencolar();
        }

        // chequeamos que estén efectivamente vacías sin nada raro adentro
        ASSERT_EQ(q.tam(), 0);
        ASSERT_EQ(p.tam(), 0);
    }
}
// lo siguientes test son tan recbucados que no los considero necesarios
const int MAX_T = 3;
